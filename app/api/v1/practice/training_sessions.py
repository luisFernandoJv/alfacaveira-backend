...
async def answer_training_question(
    ...
    return Envelope(
        data=AnswerResultResponse(
            ...
            # 🔥 CORREÇÃO: faltava repassar `teacher_name` na resposta de
            # `answer_training_question` — o schema `AnswerResultResponse`
            # já suporta o campo (usado em `_build_detail`/outras rotas),
            # só faltava passá-lo aqui também.
