...
async def main() -> None:
    async with AsyncSessionFactory() as session:
        ...
        # Usa qualquer usuário admin já existente (ex.: luisfernando.engcp@gmail.com,
        # promovido via scripts/seed_alfacaveira_admin.py) em vez de depender de um
        # e-mail fixo de teste — assim este script não fica acoplado ao
        # seed_test_data.py.
        ...
        if admin is None:
            raise RuntimeError(
                "Nenhum usuário admin encontrado. Rode scripts/seed_alfacaveira_admin.py "
                "(ou scripts/seed_test_data.py) primeiro."
            )
        print(f"Usando admin existente como created_by: {admin.email}")
